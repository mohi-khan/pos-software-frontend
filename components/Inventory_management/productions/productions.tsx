...
//             ? { ...selectedProduction, ...productionData }
//             : p
//         )
//       )
//     } else {
...
//     const matchesType =
//       typeFilter === 'All types' || production.type === typeFilter
//     const matchesStore =
//       storeFilter === 'All stores' || production.store === storeFilter
//     const matchesSearch =
//       searchTerm === '' ||
...
//                     Type
//                   </th>
//                   <th className="px-6 py-3.5 text-right text-sm font-medium text-gray-700">
//                     Quantity
//                   </th>
//                 </tr>
//               </thead>
...
// const ProductionPopup: React.FC<ProductionPopupProps> = ({
//   production,
//   onClose,
//   onSave,
// }) => {
...
const ProductionDetails: React.FC<ProductionDetailsProps> = ({
  ...
  const exportToPDF = async () => {
    const totalCost = production.items.reduce((sum, item) => sum + (item.cost * item.quantity), 0)
    ...
    try {
      ...
      script.onload = () => {
        // Wait a moment for jsPDF to fully initialize
        setTimeout(() => {
          // Check different possible ways jsPDF might be exposed
          const jsPDF = window.jsPDF?.jsPDF || window.jsPDF || (window as any).jspdf?.jsPDF
        ...
        doc.text(`Production Report - ${production.id}`, 20, yPos)
        ...
        doc.text('Production Information:', 20, yPos)
        
        yPos += 8
        doc.setFont(undefined, 'normal')
        doc.setFontSize(10)
        
        doc.text(`Date: ${production.date}`, 20, yPos)
        yPos += 6
        doc.text(`Store: ${production.store}`, 20, yPos)
        yPos += 6
        doc.text(`Type: ${production.type}`, 20, yPos)
        yPos += 6
        doc.text(`Created by: ${production.createdBy}`, 20, yPos)
        yPos += 6
        doc.text(`Notes: ${production.notes || 'N/A'}`, 20, yPos)
        ...
        doc.setFontSize(12)
        doc.setFont(undefined, 'bold')
        doc.text('Items Details:', 20, yPos)
        ...
        production.items.forEach((item, index) => {
          ...
          doc.text(item.sku, 80, yPos)
          doc.text(`Tk ${item.cost.toFixed(2)}`, 110, yPos)
          doc.text(item.quantity.toString(), 145, yPos)
          doc.text(`Tk ${itemTotal.toFixed(2)}`, 160, yPos)
        ...
        doc.setFont(undefined, 'bold')
        doc.setFontSize(10)
        doc.text('Grand Total:', 110, yPos)
        doc.text(production.quantity.toString(), 145, yPos)
        doc.text(`Tk ${totalCost.toFixed(2)}`, 160, yPos)
        
        // Save PDF
        doc.save(`Production_${production.id}.pdf`)
  ...
  const exportToCSV = () => {
    ...
    let csvContent = 'Production Report\n'
    csvContent += `Production #,${production.id}\n`
    csvContent += `Date,${production.date}\n`
    csvContent += `Store,${production.store}\n`
    csvContent += `Type,${production.type}\n`
    csvContent += `Created By,${production.createdBy}\n`
    csvContent += `Notes,${production.notes}\n`
    csvContent += '\n'
    csvContent += 'Item Name,SKU,Cost,Quantity,Total\n'
    
    production.items.forEach(item => {
      csvContent += `"${item.name}",${item.sku},${item.cost.toFixed(2)},${item.quantity},${(item.cost * item.quantity).toFixed(2)}\n`
    })
    
    csvContent += `\nTotal Cost,,,${production.quantity},${totalCost.toFixed(2)}\n`
    
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `Production_${production.id}.csv`
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                ...
                <button
                  onClick={exportToCSV}
                  className="w-full px-4 py-3 text-left hover:bg-gray-50 flex items-center gap-3 text-sm text-gray-700 transition-colors border-t border-gray-100"
                >
                  <Download size={18} />
                  Save as CSV
                </button>
