...
import { Download, Search, Calendar, ChevronLeft, ChevronRight, Store, Users, Filter, X } from 'lucide-react';
...
const InventoryHistory = () => {
  ...
  const inventoryData = [
    { date: 'Dec 31, 2025 12:55 PM', item: 'new items', store: 'bizflow', employee: 'Owner', reason: 'Transfer', adjustment: -2, stockAfter: 2 },
    { date: 'Dec 31, 2025 12:55 PM', item: 'fried rice', store: 'bizflow', employee: 'Owner', reason: 'Transfer', adjustment: -2, stockAfter: 33 },
    { date: 'Dec 30, 2025 04:56 PM', item: 'fried rice', store: 'bizflow', employee: 'Owner', reason: 'Inventory count', adjustment: -1, stockAfter: 35 },
    { date: 'Dec 30, 2025 03:23 PM', item: 'fried rice', store: 'bizflow', employee: 'Owner', reason: 'Production', adjustment: -4, stockAfter: 36 },
    { date: 'Dec 30, 2025 03:23 PM', item: 'new items', store: 'bizflow', employee: 'Owner', reason: 'Production', adjustment: 4, stockAfter: 4 },
    { date: 'Dec 30, 2025 03:03 PM', item: 'fried rice', store: 'New bizflow', employee: 'Owner', reason: 'Receive items', adjustment: 5, stockAfter: 15 },
    { date: 'Dec 30, 2025 11:30 AM', item: 'fried rice', store: 'New bizflow', employee: 'Owner', reason: 'Item edit', adjustment: 9, stockAfter: 10 },
    { date: 'Dec 30, 2025 11:29 AM', item: 'fried rice', store: 'New bizflow', employee: 'Manager', reason: 'Inventory count', adjustment: -10, stockAfter: 1 },
    { date: 'Dec 30, 2025 11:29 AM', item: 'fried rice', store: 'New bizflow', employee: 'Manager', reason: 'Receive items', adjustment: 1, stockAfter: 11 },
    { date: 'Dec 30, 2025 11:26 AM', item: 'fried rice', store: 'New bizflow', employee: 'Staff', reason: 'Item edit', adjustment: 10, stockAfter: 10 },
  ];

  const stores = ['All stores', 'bizflow', 'New bizflow'];
  const employees = ['All employees', 'Owner', 'Manager', 'Staff'];
  const reasons = ['All reasons', 'Transfer', 'Inventory count', 'Production', 'Receive items', 'Item edit'];
  ...
  const handleQuickDate = (value: string) => {
    ...
    setStartDate(`${start.getMonth() + 1}/${start.getDate()}/${start.getFullYear()}`);
  ...
  const handleDayClick = (day: number, month: number, year: number) => {
    ...
    if (selectedStartDay && !selectedEndDay) {
      const startDateObj = new Date(selectedStartYear, selectedStartMonth, selectedStartDay);
      ...
      if (endDateObj >= startDateObj) {
        ...
        setEndDate(`${selectedStartMonth + 1}/${selectedStartDay}/${selectedStartYear}`);
        setSelectedStartDay(day);
        setSelectedStartMonth(month);
        setSelectedStartYear(year);
        setStartDate(`${month + 1}/${day}/${year}`);
  ...
  const isInRange = (day: number, month: number, year: number) => {
    ...
    const startDateObj = new Date(selectedStartYear, selectedStartMonth, selectedStartDay);
    const endDateObj = new Date(selectedEndYear, selectedEndMonth, selectedEndDay);
    
    return currentDate >= startDateObj && currentDate <= endDateObj;
  };

  const isStartOrEnd = (day: number, month: number, year: number) => {
    const isStart = day === selectedStartDay && month === selectedStartMonth && year === selectedStartYear;
    const isEnd = day === selectedEndDay && month === selectedEndMonth && year === selectedEndYear;
  ...
  const filteredData = inventoryData.filter(row => {
    ...
    const matchesStore = selectedStore === 'All stores' || row.store === selectedStore;
    const matchesEmployee = selectedEmployee === 'All employees' || row.employee === selectedEmployee;
    const matchesReason = selectedReason === 'All reasons' || row.reason.includes(selectedReason);
                    ...
                    {stores.map(store => (
                      <button
                        ...
                        className={`block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm ${
                          selectedStore === store ? 'bg-gray-50 font-medium' : ''
                        }`}
                      >
                        {store}
                      </button>
                  ...
                  <div className="absolute top-full mt-1 left-0 bg-white border rounded-lg shadow-lg z-10 min-w-[160px]">
                    {employees.map(employee => (
                      <button
                        ...
                        className={`block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm ${
                          selectedEmployee === employee ? 'bg-gray-50 font-medium' : ''
                        }`}
                      >
                        {employee}
                      </button>
                  ...
                  <div className="absolute top-full mt-1 left-0 bg-white border rounded-lg shadow-lg z-10 min-w-[160px]">
                    {reasons.map(reason => (
                      <button
                        ...
                        className={`block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm ${
                          selectedReason === reason ? 'bg-gray-50 font-medium' : ''
                        }`}
                      >
                        {reason}
                      </button>
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-7 gap-1 mb-4">
                  ...
                  {generateCalendarDays().map((dayObj, idx) => {
                    const isSelectedDate = dayObj.isCurrentMonth && isStartOrEnd(dayObj.day, dayObj.month, dayObj.year);
                    const inRange = dayObj.isCurrentMonth && isInRange(dayObj.day, dayObj.month, dayObj.year) && !isSelectedDate;
                    ...
                    return (
                      <button
                        ...
                        className={`p-2 text-sm rounded-full transition-colors ${
                          isSelectedDate ? 'bg-green-500 text-white hover:bg-green-600' : 
                          inRange ? 'bg-green-100 text-green-700 hover:bg-green-200' :
                          dayObj.isCurrentMonth ? 'hover:bg-gray-100' :
                          'text-gray-400'
                        }`}
                      >
                        {dayObj.day}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-max table-fixed">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-600 w-48">Date</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-600 w-32">Item</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-600 w-32">Store</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-600 w-32">Employee</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-gray-600 w-56">Reason</th>
                  <th className="text-right px-6 py-3 text-xs font-medium text-gray-600 w-28">Adjustment</th>
                  <th className="text-right px-6 py-3 text-xs font-medium text-gray-600 w-28">Stock after</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {filteredData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900 truncate">{row.item}</td>
                    <td className="px-6 py-4 text-sm text-gray-900 truncate">{row.store}</td>
                    <td className="px-6 py-4 text-sm text-gray-900 truncate">{row.employee}</td>
                    <td className="px-6 py-4 text-sm truncate">
                      <span className={getReasonColor(row.reason)}>{row.reason}</span>
                    </td>
                    <td className={`px-6 py-4 text-sm text-right whitespace-nowrap ${row.adjustment > 0 ? 'text-green-600' : 'text-gray-900'}`}>
                      {row.adjustment}
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-gray-900 whitespace-nowrap">{row.stockAfter}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div key={idx} className="bg-white rounded-lg shadow-sm p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="text-right">
                  <div className={`font-medium ${row.adjustment > 0 ? 'text-green-600' : 'text-gray-900'}`}>
                    {row.adjustment > 0 ? '+' : ''}{row.adjustment}
                  </div>
                  <div className="text-xs text-gray-500">Stock: {row.stockAfter}</div>
                </div>
              </div>
